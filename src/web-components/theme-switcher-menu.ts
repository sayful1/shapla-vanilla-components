type TYPE_COLOR_SCHEME = 'os-default' | 'dark' | 'light';

class ShaplaThemeSwitcherMenu extends HTMLElement {
  ...
  constructor() {
    ...
    style.textContent = ShaplaThemeSwitcherMenu.getStyle();

    // attach the created elements to the shadow DOM
    (this.shadowRoot as ShadowRoot).append(style, this.getElement());

    const triggerEl = (this.shadowRoot as ShadowRoot).querySelector('.trigger');
    if (triggerEl) {
      triggerEl.addEventListener('click', () => {
        this.toggleActive();
      })
    }
    (this.shadowRoot as ShadowRoot).querySelectorAll<HTMLElement>('.item').forEach((item: HTMLElement) => {
      item.addEventListener('click', () => {
        const theme = item.dataset.option as TYPE_COLOR_SCHEME;
   ...
   */
  toggleActive(): void {
    const root = (this.shadowRoot as ShadowRoot).querySelector<HTMLDivElement>('.menu');
    if (root) {
   ...
   */
  updateIcon(theme: TYPE_COLOR_SCHEME) {
    const icon = (this.shadowRoot as ShadowRoot).querySelector<HTMLSpanElement>('.trigger-icon');
    if (icon) {
      icon.innerHTML = this.themeSwitcherIcon(theme);
    }
  }
   ...
   * @param {string} theme The color theme slug.
   */
  changeTheme(theme: TYPE_COLOR_SCHEME) {
    if (this.colorSchemes.includes(theme)) {
   ...
   * @returns {HTMLElement}
   */
  getMenuItem(icon: string, label: string): HTMLSpanElement {
   ...
   * @returns {string}
   */
  themeSwitcherIcon(icon: string): string {
...
.trigger {display: flex;align-items: center; color: var(--shapla-text-primary,#000000de)}
...
color: var(--shapla-text-primary,#000000de); padding-bottom: 0.5em; padding-top: 0.5em;}`;
   ...
   * @returns {HTMLElement}
   */
  el(tagName: string, attributes: Record<string, string> = {}, children: (string | HTMLElement)[] = []): HTMLElement {
...
export default ShaplaThemeSwitcherMenu;
